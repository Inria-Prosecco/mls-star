...
val ps_leaf_package_content: #bytes:Type0 -> {|bytes_like bytes|} -> parser_serializer bytes (leaf_package_content_nt bytes)
let ps_leaf_package_content #bytes #bl =
  mk_isomorphism (leaf_package_content_nt bytes)
    ps_hpke_public_key
...
instance parseable_serializeable_leaf_package_content_nt (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (leaf_package_content_nt bytes) =
  mk_parseable_serializeable ps_leaf_package_content
...
val ps_node_package_content: #bytes:Type0 -> {|bytes_like bytes|} -> parser_serializer bytes (node_package_content_nt bytes)
let ps_node_package_content #bytes #bl =
  let open Comparse in
  mk_isomorphism (node_package_content_nt bytes)
    (
      _ <-- ps_hpke_public_key;
      bind (ps_blseq _ ps_hpke_ciphertext) (fun (_:blseq (hpke_ciphertext_nt bytes) ps_hpke_ciphertext ({min=0; max=(pow2 32)-1})) -> //See FStarLang/FStar#2589
      ps_blbytes _
      )
    )
...
instance parseable_serializeable_node_package_content_nt (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (node_package_content_nt bytes) =
  mk_parseable_serializeable ps_node_package_content
...
let key_package_to_treesync #bytes #bl kp =
  ...
  | C_basic cred ->
    return ({
      ...
      TS.version = 0;
      TS.content = serialize (leaf_package_content_nt bytes) ({public_key = kp.public_key});
      TS.extensions = (ps_to_pse (ps_blseq _ ps_extension)).serialize_exact (kp.extensions);
...
let treesync_to_keypackage #bytes #cb lp =
  if not (length lp.TS.credential.TS.identity < pow2 16) then
    ...
    extensions <-- from_option "treesync_to_keypackage: can't parse extensions" ((ps_to_pse (ps_blseq _ ps_extension)).parse_exact lp.TS.extensions);
...
let parent_node_to_treesync #bytes #bl pn =
  bytes_length_nil #bytes ps_hpke_ciphertext;
  return ({
        TS.version = 0;
...
let key_package_to_treekem #bytes #cb kp =
  if not (length (kp.public_key <: bytes) = hpke_public_key_length #bytes) then
...
let rec update_path_to_treekem #bytes #cb l n i group_context update_path =
  if l = 0 then (
    ...
    if not (Seq.length update_path.nodes > 0) then
      ...
      assume (bytes_length ps_update_path_node (Seq.seq_to_list tail_update_path_nodes) <= bytes_length ps_update_path_node (Seq.seq_to_list update_path.nodes));
...
let treesync_to_update_path #bytes #cb #l #n #i p =
  ...
  if not (bytes_length ps_update_path_node upns < pow2 32) then
...
val ratchet_tree_to_treesync: #bytes:Type0 -> {|bytes_like bytes|} -> l:nat -> n:tree_size l -> nodes:Seq.seq (option_nt (node_nt bytes)){Seq.length nodes = (n+n-1)} -> result (TS.treesync bytes l n)
...
val treesync_to_ratchet_tree: #bytes:Type0 -> {|crypto_bytes bytes|} -> #l:nat -> #n:tree_size l -> TS.treesync bytes l n -> result (Seq.seq (option_nt (node_nt bytes)))
