...
let ps_hpke_public_key_nt #bytes #bl = ps_blbytes _
...
val ps_key_package_ref_nt: #bytes:Type0 -> {|bytes_like bytes|} -> parser_serializer bytes (key_package_ref_nt bytes)
let ps_key_package_ref_nt #bytes #bl = ps_blbytes _
...
let ps_certificate_nt #bytes #bl = ps_blbytes _
...
noeq type credential_nt (bytes:Type0) {|bytes_like bytes|} =
  ...
  | C_x509: [@@@ with_num_tag 2 2] blseq bytes ps_certificate_nt ({min=1; max=(pow2 32)-1}) -> credential_nt bytes
...
noeq type key_package_tbs_nt (bytes:Type0) {|bytes_like bytes|} = {
  ...
  extensions: blseq bytes ps_extension_nt ({min=8; max=(pow2 32)-1});
}
...
noeq type update_path_nt (bytes:Type0) {|bytes_like bytes|} = {
  ...
  nodes: blseq bytes ps_update_path_node_nt ({min=0; max=(pow2 32)-1});
}
...
noeq type group_context_nt (bytes:Type0) {|bytes_like bytes|} = {
  ...
  extensions: blseq bytes ps_extension_nt ({min=0; max=(pow2 32)-1});
}
...
noeq type parent_node_nt (bytes:Type0) {|bytes_like bytes|} = {
  ...
  unmerged_leaves: blseq bytes (ps_nat_lbytes #bytes 4) ({min=0; max=(pow2 32)-1});
}
...
type ratchet_tree_nt (bytes:Type0) {|bytes_like bytes|} = blseq bytes (ps_option ps_node_nt) ({min=1; max=(pow2 32)-1})

val ps_ratchet_tree_nt: #bytes:Type0 -> {|bytes_like bytes|} -> parser_serializer bytes (ratchet_tree_nt bytes)
let ps_ratchet_tree_nt #bytes #bl = ps_blseq (ps_option ps_node_nt) _
...
noeq type pre_shared_key_id_nt (bytes:Type0) {|bytes_like bytes|} =
  //| PSKI_reserved: [@@@ with_tag (PSKT_reserved ())] psk_nonce:blbytes bytes ({min=0; max=255}) -> pre_shared_key_id_nt bytes
  | PSKI_external: [@@@ with_tag (PSKT_external ())] psk_id:blbytes bytes ({min=0; max=255}) -> psk_nonce:blbytes bytes ({min=0; max=255}) -> pre_shared_key_id_nt bytes
  | PSKI_reinit: [@@@ with_tag (PSKT_reinit ())] psk_group_id:blbytes bytes ({min=0; max=255}) -> psk_epoch:nat_lbytes 8 -> psk_nonce:blbytes bytes ({min=0; max=255}) -> pre_shared_key_id_nt bytes
  | PSKI_branch: [@@@ with_tag (PSKT_branch ())] psk_group_id:blbytes bytes ({min=0; max=255}) -> psk_epoch:nat_lbytes 8 -> psk_nonce:blbytes bytes ({min=0; max=255}) -> pre_shared_key_id_nt bytes

%splice [ps_pre_shared_key_id_nt] (gen_parser (`pre_shared_key_id_nt))

noeq type pre_shared_keys_nt (bytes:Type0) {|bytes_like bytes|} = {
  psks: blseq bytes ps_pre_shared_key_id_nt ({min=0; max=(pow2 16)-1});
}
...
noeq type app_ack_nt (bytes:Type0) {|bytes_like bytes|} = {
  received_ranges: blseq bytes ps_message_range_nt ({min=0; max=(pow2 32)-1})
}
...
noeq type commit_nt (bytes:Type0) {|bytes_like bytes|} = {
  proposals: blseq bytes ps_proposal_or_ref_nt ({min=0; max=(pow2 32)-1});
...
noeq type sender_nt (bytes:Type0) {|bytes_like bytes|} =
  ...
  | S_preconfigured: [@@@ with_tag (ST_preconfigured ())] external_key_id:blbytes bytes ({min=0; max=255}) -> sender_nt bytes
...
noeq type welcome_nt (bytes:Type0) {|bytes_like bytes|} = {
  version: protocol_version_nt;
  cipher_suite: cipher_suite_nt;
  secrets: blseq bytes ps_encrypted_group_secrets_nt ({min=0; max=(pow2 32)-1});
