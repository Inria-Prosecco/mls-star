...
val create_tree: #bytes:Type0 -> {|bytes_like bytes|} -> leaf_package_t bytes -> treesync bytes 0
let create_tree lp =
  TLeaf (Some lp)

(*** Tree operations ***)

val tree_add: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> nat -> i:leaf_index l -> leaf_package_t bytes -> treesync bytes l
let rec tree_add #bytes #bl #l t original_i i lp =
  match t with
...
val tree_blank_path: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> i:leaf_index l -> option (leaf_package_t bytes) -> treesync bytes l
let rec tree_blank_path #bytes #bl #l t i opt_lp =
  match t with
...
val tree_update: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> i:leaf_index l -> leaf_package_t bytes -> treesync bytes l
let tree_update #bytes #bl #l t i lp =
  tree_blank_path t i (Some lp)

val tree_remove: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> i:leaf_index l -> treesync bytes l
let tree_remove #bytes #bl #l t i =
  tree_blank_path t i None

val apply_path: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> pathsync bytes l -> leaf_index l -> treesync bytes l
let rec apply_path #bytes #bl #l t p i =
  match t,p with
  | TLeaf _, PLeaf m' -> TLeaf m'
  | TNode _ left right, PNode onp next ->
     let (dir,new_i) = child_index l i in
     if dir = Left
     then TNode onp (apply_path left next new_i) right
     else TNode onp left (apply_path right next new_i)


(*** Tree extension / truncation ***)

val is_tree_empty: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> bool
let rec is_tree_empty #bytes #bl #l t =
  match t with
  | TNode _ left right ->
...
val canonicalize_tree: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> (l_res:nat{l_res <= l} & treesync bytes l_res)
...
val find_empty_leaf: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> option (leaf_index l)
let rec find_empty_leaf #bytes #bl #l t =
  match t with
  | TNode _ left right -> (
...
val mk_blank_tree: #bytes:Type0 -> {|bytes_like bytes|} -> l:nat -> Pure (treesync bytes l) (requires True) (ensures fun res -> Some? (find_empty_leaf res))
let rec mk_blank_tree #bytes #bl l =
  if l = 0 then TLeaf None
  else TNode None (mk_blank_tree (l-1)) (mk_blank_tree (l-1))

val add_one_level: #bytes:Type0 -> {|bytes_like bytes|} -> #l:nat -> treesync bytes l -> Pure (treesync bytes (l+1)) (requires True) (fun res -> Some? (find_empty_leaf res))
let add_one_level #bytes #bl #l t =
  TNode None t (mk_blank_tree l)
...
val state_update_tree: #bytes:Type0 -> {|bytes_like bytes|} -> #l:level_n -> state_t bytes -> treesync bytes l -> state_t bytes
