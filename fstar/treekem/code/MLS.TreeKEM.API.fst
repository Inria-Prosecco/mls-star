...
val commit:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #leaf_ind:nat ->
  st:treekem_state bytes leaf_ind -> #li:treekem_index st{li <> leaf_ind} -> p:pathkem bytes st.levels 0 li{path_filtering_ok st.tree p} -> excluded_leaves:list nat{~(List.Tot.memP leaf_ind excluded_leaves)} -> group_context_nt bytes ->
