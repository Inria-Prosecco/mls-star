...
let as_tokens (bytes:Type0) {|bytes_like bytes|} (asp:as_parameters bytes) = tree (option asp.token_t) unit

val as_add_update: #bytes:Type0 -> {|bytes_like bytes|} -> #asp:as_parameters bytes -> #l:nat -> #i:tree_index l -> as_tokens bytes asp l i -> leaf_index l i -> asp.token_t -> as_tokens bytes asp l i
let as_add_update #bytes #bl #asp #l #i t li token =
  tree_change_path t li (Some token) ()

val as_remove: #bytes:Type0 -> {|bytes_like bytes|} -> #asp:as_parameters bytes -> #l:nat -> #i:tree_index l -> as_tokens bytes asp l i -> leaf_index l i -> as_tokens bytes asp l i
let as_remove #bytes #bl #asp #l #i t li =
  tree_change_path t li None ()

val as_truncate: #bytes:Type0 -> {|bytes_like bytes|} -> #asp:as_parameters bytes -> #l:pos -> as_tokens bytes asp l 0 -> as_tokens bytes asp (l-1) 0
let as_truncate #bytes #bl #asp #l t =
  let TNode _ left _ = t in
  left

val as_extend: #bytes:Type0 -> {|bytes_like bytes|} -> #asp:as_parameters bytes -> #l:nat -> as_tokens bytes asp l 0 -> as_tokens bytes asp (l+1) 0
let as_extend #bytes #bl #asp #l t =
  TNode () t (mk_blank_tree_general _ _ None ())

val one_credential_ok: #bytes:Type0 -> {|bytes_like bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes -> #l:nat -> #i:tree_index l -> treesync bytes tkt l i -> as_tokens bytes asp l i -> li:leaf_index l i -> prop
...
val all_credentials_ok: #bytes:Type0 -> {|bytes_like bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes -> #l:nat -> #i:tree_index l -> treesync bytes tkt l i -> as_tokens bytes asp l i -> prop
