...
let message_compute_auth cs msg sk rand group_context confirmation_key interim_transcript_hash =
  signature <-- compute_message_signature cs sk rand msg group_context;
  confirmation_tag <-- (
    if msg.content_type = Content.CT_commit then (
      confirmed_transcript_hash <-- compute_confirmed_transcript_hash cs msg signature interim_transcript_hash;
