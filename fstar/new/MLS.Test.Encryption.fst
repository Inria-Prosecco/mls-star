...
let test_leaf_generation #cs sender_data_secret r_state test =
  ...
  let message_2 = extract_result (message_ciphertext_to_message (fun i -> if r_state.generation <= i then ratchet_get_generation_key r_state i else internal_failure "ratchet: generation not available") sender_data_secret message_ciphertext) in
...
let test_leaf cs l n i encryption_secret sender_data_secret test =
  let leaf_encryption_secret = extract_result (leaf_kdf n cs encryption_secret (MLS.TreeMath.root l) i) in
    ...
    | Success cs -> begin
      ...
      let leaves_ok =
        if List.Tot.length t.leaves <= n_leaves then
          test_leaves cs l n_leaves (hex_string_to_bytes t.encryption_secret) (hex_string_to_bytes t.sender_data_secret) t.leaves
        else
