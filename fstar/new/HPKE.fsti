...
//Adapted from https://github.com/project-everest/hacl-star/blob/_blipp_hpke/specs/Spec.Agile.HPKE.fsti
//TODO: remove this file once hacl-star-snapshot is updated
...
let hkdf_extract_ikm_length_pred (a:hash_algorithm) (ikm_length:nat) =
  ikm_length + Hash.block_length a <= Hash.max_input_length a

let hkdf_expand_info_length_pred (a:hash_algorithm) (info_length:nat) =
  Hash.hash_length a + info_length + 1 + Hash.block_length a <= Hash.max_input_length a

let hkdf_expand_output_length_pred (a:hash_algorithm) (len:nat) =
  len <= 255 * Hash.hash_length a


let labeled_extract_ikm_length_pred (a:hash_algorithm) (ikm_length:nat) =
  hkdf_extract_ikm_length_pred a (size_label_version + ikm_length)
...
let labeled_expand_info_length_pred (a:hash_algorithm) (info_length:nat) =
  hkdf_expand_info_length_pred a (2 + size_label_version + info_length)
...
val labeled_expand:
  ...
  Pure (lbytes l)
    (requires
      ...
      hkdf_expand_output_length_pred a l)
    (ensures fun _ -> True)
...
let exp_len (cs:ciphersuite) = l:size_nat{hkdf_expand_output_length_pred (hash_of_cs cs) l}
