...
val state_leaf_at:
  ...
  st:treesync_state bytes tkt token_t group_id -> li:treesync_index st{Some? (leaf_at st.tree li)} ->
  leaf_node_nt bytes tkt
let state_leaf_at #bytes #cb #tkt #token_t #group_id st li =
  Some?.v (leaf_at st.tree li)

val state_update_tree:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #tkt:treekem_types bytes -> #token_t:Type0 -> #group_id:mls_bytes bytes ->
  #l:nat ->
  st:treesync_state bytes tkt token_t group_id -> new_tree:treesync_valid bytes tkt l 0 group_id -> new_tokens:as_tokens bytes token_t l 0 ->
  treesync_state bytes tkt token_t group_id
let state_update_tree #bytes #cb #tkt #token_t #group_id #l st new_tree new_tokens =
  ({
    levels = l;
    tree = new_tree;
    tokens = new_tokens;
...
val finalize_create_valid:
  ...
  #group_id:mls_bytes bytes -> #ln:leaf_node_nt bytes tkt ->
  pend:pending_create group_id ln -> token:asp.token_t ->
  Lemma
  (requires asp.credential_ok pend.as_input token)
  (ensures treesync_state_valid asp (finalize_create pend token))
let finalize_create_valid #bytes #cb #tkt #asp #group_id #ln pend token =
...
val tokens_from_list:
  ...
  l:nat -> i:tree_index l -> tokens:list (option token_t){List.Tot.length tokens == pow2 l} ->
  as_tokens bytes token_t l i
let rec tokens_from_list #bytes #bl #token_t l i tokens =
  if l = 0 then (
    let [token] = tokens in
    TLeaf token
  ) else (
    ...
    TNode () (tokens_from_list #bytes _ _ tokens_left) (tokens_from_list #bytes _ _ tokens_right)
  )
#pop-options
...
val finalize_welcome:
  ...
  treesync_state bytes tkt token_t group_id
let finalize_welcome #bytes #cb #tkt #token_t #l #group_id #t pend tokens =
  pend.can_welcome_proof;
  let tokens_tree = tokens_from_list l 0 tokens in
  ({
    levels = l;
    tree = t;
    tokens = tokens_tree;
...
val leaf_at_token_from_list:
  #bytes:Type0 -> {|bytes_like bytes|} -> #token_t:Type0 ->
  l:nat -> i:tree_index l -> tokens:list (option token_t){List.Tot.length tokens == pow2 l} -> li:leaf_index l i ->
  Lemma
  (leaf_at (tokens_from_list #bytes l i tokens) li == List.Tot.index tokens (li-i))
let rec leaf_at_token_from_list #bytes #bl #token_t l i tokens li =
  if l = 0 then ()
  else (
    let tokens_left, tokens_right = List.Tot.splitAt (pow2 (l-1)) tokens in
    ...
    if is_left_leaf li then
      leaf_at_token_from_list #bytes (l-1) (left_index i) tokens_left li
    else
      leaf_at_token_from_list #bytes (l-1) (right_index i) tokens_right li
  )
#pop-options

val finalize_welcome_valid:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes ->
  #l:nat -> #group_id:mls_bytes bytes -> #t:treesync bytes tkt l 0 ->
  pend:pending_welcome group_id t -> tokens:list (option asp.token_t){List.Tot.length tokens == pow2 l} ->
  Lemma
  (requires
  ...
  (ensures treesync_state_valid asp (finalize_welcome pend tokens))
let finalize_welcome_valid #bytes #cb #tkt #asp #l #group_id #t pend tokens =
  let tokens_tree = tokens_from_list l 0 tokens in
  intro_all_credentials_ok t tokens_tree (fun li ->
    pend.as_inputs_proof;
    leaf_at_token_from_list #bytes l 0 tokens li
  )
...
let prepare_add #bytes #cb #tkt #token_t #group_id st ln =
...
val finalize_add:
  ...
  result (treesync_state bytes tkt token_t group_id & nat)
let finalize_add #bytes #cb #tkt #token_t #group_id #st #ln pend token =
  pend.can_add_proof;
  match find_empty_leaf st.tree with
  | Some li -> (
...
val finalize_add_valid:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes -> #group_id:mls_bytes bytes ->
  #st:treesync_state bytes tkt asp.token_t group_id -> #ln:leaf_node_nt bytes tkt ->
  pend:pending_add st ln -> token:asp.token_t ->
  Lemma
  (requires
    asp.credential_ok pend.as_input token /\
    treesync_state_valid asp st
  )
  (ensures (
    match finalize_add pend token with
    | Success (res, _) -> treesync_state_valid asp res
    | _ -> True
  ))
let finalize_add_valid #bytes #cb #tkt #asp #group_id #st #ln pend token =
  pend.can_add_proof;
  match finalize_add pend token with
  | Success _ -> (
    match find_empty_leaf st.tree with
...
let prepare_update #bytes #cb #tkt #token_t #group_id st ln li =
...
val finalize_update:
  ...
  treesync_state bytes tkt token_t group_id
let finalize_update #bytes #cb #tkt #token_t #group_id #st #ln #li pend token =
  pend.can_update_proof;
  (state_update_tree st (tree_update st.tree li ln) (as_add_update st.tokens li token))

val finalize_update_valid:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes -> #group_id:mls_bytes bytes ->
  #st:treesync_state bytes tkt asp.token_t group_id -> #ln:leaf_node_nt bytes tkt -> #li:treesync_index st ->
  pend:pending_update st ln li -> token:asp.token_t ->
  Lemma
  (requires
    asp.credential_ok pend.as_input token /\
    treesync_state_valid asp st
  )
  (ensures treesync_state_valid asp (finalize_update pend token))
let finalize_update_valid #bytes #cb #tkt #asp #group_id #st #ln #li pend token =
...
let prepare_remove #bytes #cb #tkt #token_t #group_id st li =
...
val fully_truncate_state:
  ...
  Tot (treesync_state bytes tkt token_t group_id)
  (decreases st.levels)
let rec fully_truncate_state #bytes #cb #tkt #token_t #group_id st =
...
val finalize_remove:
  ...
  treesync_state bytes tkt token_t group_id
let finalize_remove #bytes #cb #tkt #token_t #group_id #st #li pend =
...
val fully_truncate_state_valid:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes -> #group_id:mls_bytes bytes ->
  st:treesync_state bytes tkt asp.token_t group_id ->
  Lemma
  (requires treesync_state_valid asp st)
  (ensures treesync_state_valid asp (fully_truncate_state st))
  (decreases st.levels)
let rec fully_truncate_state_valid #bytes #cb #tkt #asp #group_id st =
  if 1 <= st.levels && is_tree_empty (TNode?.right st.tree) then (
    all_credentials_ok_tree_truncate st.tree st.tokens;
    fully_truncate_state_valid {
      levels = st.levels-1;
      tree = tree_truncate st.tree;
      tokens = as_truncate st.tokens;
    }
...
val finalize_remove_valid:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes -> #group_id:mls_bytes bytes ->
  #st:treesync_state bytes tkt asp.token_t group_id -> #li:treesync_index st ->
  pend:pending_remove st li ->
  Lemma
  (requires treesync_state_valid asp st)
  (ensures treesync_state_valid asp (finalize_remove pend))
let finalize_remove_valid #bytes #cb #tkt #asp #group_id #st #li pend =
  let blanked_tree = tree_remove st.tree li in
  let blanked_tokens = as_remove st.tokens li in
  all_credentials_ok_tree_remove st.tree st.tokens li;
  fully_truncate_state_valid (state_update_tree st blanked_tree blanked_tokens)
...
let prepare_commit #bytes #cb #tkt #token_t #group_id st #li p =
...
val finalize_commit:
  ...
  result (treesync_state bytes tkt token_t group_id)
let finalize_commit #bytes #cb #tkt #token_t #group_id #st #li #p pend token =
...
#push-options "--fuel 0 --ifuel 1"
val finalize_commit_valid:
  #bytes:Type0 -> {|crypto_bytes bytes|} -> #tkt:treekem_types bytes -> #asp:as_parameters bytes -> #group_id:mls_bytes bytes ->
  #st:treesync_state bytes tkt asp.token_t group_id -> #li:treesync_index st -> #p:pathsync bytes tkt st.levels 0 li ->
  pend:pending_commit st p -> token:asp.token_t ->
  Lemma
  (requires
    asp.credential_ok pend.as_input token /\
    treesync_state_valid asp st
  ...
  (ensures (
    ...
    | Success res -> treesync_state_valid asp res
    | _ -> True
  ))
let finalize_commit_valid #bytes #cb #tkt #asp #group_id #st #li #p pend token =
  match finalize_commit pend token with
  | Success _ ->
...
let authenticate_external_path #bytes #cb #tkt #token_t #group_id st #li p sign_private_key sign_nonce =
...
val compute_tree_hash:
  ...
  treesync_state bytes tkt token_t group_id ->
  result bytes
let compute_tree_hash #bytes #cb #tkt #token_t #group_id st =
  tree_hash st.tree
...
val compute_provisional_tree_hash:
  ...
  result bytes
let compute_provisional_tree_hash #bytes #cb #tkt #token_t #group_id st #li p =
