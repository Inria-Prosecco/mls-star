...
let test_leaf_generation #cb l i encryption_secret sender_data_secret r_state test =
  ...
  let message_plaintext = extract_option "bad plaintext" ((ps_prefix_to_ps_whole ps_public_message_nt).parse (hex_string_to_bytes plaintext_string)) in
  let message_ciphertext = extract_option "bad ciphertext" ((ps_prefix_to_ps_whole ps_private_message_nt).parse (hex_string_to_bytes ciphertext_string)) in
  ...
  let message_2 = ({ message_2 with wire_format = WF_mls_public_message}) in
