...
let send_helper st msg e =
  ...
  let? auth = message_auth_data wire_format msg st.sign_private_key rand_nonce (if knows_group_context msg.sender then group_context else ()) confirmation_secret st.interim_transcript_hash in
...
let process_group_message state msg =
  ...
  let? (wire_format, message) = (
    ...
    | M_mls10 (M_public_message msg) ->
        ...
        let? auth_msg = public_message_to_authenticated_content msg (if S_member? msg.content.sender then group_context else ()) (if S_member? msg.content.sender then membership_key else ()) in
